...
public partial class IDMainPageNew : ContentPage
{
    ...
    public IDMainPageNew()
	{
        ...
        zTGR.Tapped += async (s, e) =>
        {
            ...
            if (s1.Source != null)
            {
                ...
                //await Shell.Current.Navigation.PushModalAsync(new NavigationPage(new ImageZoomModalPage(s1.Source)), false);
                ...
                //await Shell.Current.Navigation.PushAsync(new ImageZoomPage(s1.Source), false);
                //await Navigation.PushAsync(new ImageZoomPage(s1.Source), false);
            }
        };
        ...
        if (Application.Current != null)
        {
            ...
            {
                if (ImageLabel0.FormattedText?.Spans.Count > 0)
                {
                    for (int i = 0; i < ImageLabel0.FormattedText.Spans.Count; i++)
                    {
                        if (Application.Current.RequestedTheme == AppTheme.Dark) ImageLabel0.FormattedText.Spans[i].TextColor = Colors.White;
                        else ImageLabel0.FormattedText.Spans[i].TextColor = Colors.Black;
                    }
                }
                if (ImageLabel1.FormattedText?.Spans.Count > 0)
                {
                    for (int i = 0; i < ImageLabel1.FormattedText.Spans.Count; i++)
                    {
                        if (Application.Current.RequestedTheme == AppTheme.Dark) ImageLabel1.FormattedText.Spans[i].TextColor = Colors.White;
                        else ImageLabel1.FormattedText.Spans[i].TextColor = Colors.Black;
                    }
                }
                if (ImageLabel2.FormattedText?.Spans.Count > 0)
                {
                    for (int i = 0; i < ImageLabel2.FormattedText.Spans.Count; i++)
                    {
                        if (Application.Current.RequestedTheme == AppTheme.Dark) ImageLabel2.FormattedText.Spans[i].TextColor = Colors.White;
                        else ImageLabel2.FormattedText.Spans[i].TextColor = Colors.Black;
                    }
                }
            };
        }
    ...
    protected override async void OnAppearing()
    {
        ...
        if (!isCreated || fsChanged || langChanged)
        {
            ...
            localBoxes[0].Color = Colors.White;
            localGrids[0].BackgroundColor = Colors.LightGrey;
        ...
        if (currentDecision == 0 || currentDecision == entID)
        {
            ...
            if (entryTarget <= 0)
            {
                ...
                if (index.EndsWith('a'))
                {
                    ...
                    //localBoxes[entID - 1].Color = Colors.White;
                    //localGrids[entID - 1].BackgroundColor = Colors.LightGrey;

                    //Show images for selected terminal decision if they are not already selected
                    if (currentImages != entID.ToString() + "a") OnButtonClicked(localButtons[entID - 1], new EventArgs());
                ...
                else if (index.EndsWith('b'))
                {
                    ...
                    //localBoxes[entID - 1].Color = Colors.LightGrey;
                    //localGrids[entID - 1].BackgroundColor = Colors.White;

                    //Show images for selected terminal decision if they are not already selected
                    if (currentImages != entID.ToString() + "b") OnButtonClicked1(localButtons1[entID - 1], new EventArgs());
                    else HapticFeedback.Default.Perform(HapticFeedbackType.Click);
            ...
            else
            {
                ...
                for (int i = keyTarget + 1; i < 94; i++)
                {
                    localBoxes[i].Color = Color.FromArgb("#BDE6E2");
                    localGrids[i].BackgroundColor = Color.FromArgb("#BDD6E6");
                }

                //if (ImgRD.Height != 0) localBoxes[keyTarget].Color = Colors.White;
                //else localBoxes[keyTarget].Color = Color.FromArgb("#BDE6E2");
                //localGrids[keyTarget].BackgroundColor = Color.FromArgb("#BDD6E6");
                localBoxes[keyTarget].Color = Colors.White;
                localGrids[keyTarget].BackgroundColor = Colors.LightGrey;
    ...
    void OnTGRTapped(object? sender, TappedEventArgs args)
    {
        ...
        if (currentImg == 'a')
        {
            ...
            if (ImgRD.Height != 0 || ZoomRD.Height != 0)
            {
                localBoxes[entID - 1].Color = Colors.LightGrey;
                localGrids[entID - 1].BackgroundColor = Colors.White;
            }
            ...
            ImageLabel0.FormattedText = formattedImageNames[0];
            Image0.Source = ImageSource.FromFile(fileNames[0]);
            if (imgZoom) Image0Z.Source = ImageSource.FromFile(fileNames[0]);
            if (fileNames.Count > 1)
            {
                ImageLabel1.FormattedText = formattedImageNames[1];
                Image1.Source = ImageSource.FromFile(fileNames[1]);
                if (imgZoom) Image1Z.Source = ImageSource.FromFile(fileNames[1]);
            }
            else
            {
                Image1.Source = null;
            ...
            if (fileNames.Count > 2)
            {
                ImageLabel2.FormattedText = formattedImageNames[2];
                Image2.Source = ImageSource.FromFile(fileNames[2]);
                if (imgZoom) Image2Z.Source = ImageSource.FromFile(fileNames[2]);
            }
            else
            {
                Image2.Source = null;
    ...
    private async void OnImagesButtonClicked(object sender, EventArgs args)
    {
        ...
        if (imgZoom)
        {
            if (ZoomRD.Height == 0)
            {
                ...
                if (currentDecision == currentInd)
                {
                    localBoxes[currentInd - 1].Color = Colors.White;
                    localGrids[currentInd - 1].BackgroundColor = Colors.LightGrey;
                }
                else
                {
                    if (currentInd != 1)
            ...
            else
            {
                ...
                //else if (currentImg == 'b')
                //{
                //    localBoxes[currentInd - 1].Color = Colors.LightGrey;
                //    localGrids[currentInd - 1].BackgroundColor = Colors.White;
                //}
                if (currentDecision == currentInd)
                {
                    //localBoxes[currentInd - 1].Color = Colors.White;
        ...
        else
        {
            if (ImgRD.Height == 0)
            {
                ...
                if (currentDecision == 0 && currentInd != 1)
                {
                    localBoxes[0].Color = Color.FromArgb("#BDE6E2");
                    localGrids[0].BackgroundColor = Color.FromArgb("#BDD6E6");
                }
    ...
    private async void ClearID_Clicked(object sender, EventArgs e)
    {
        HapticFeedback.Default.Perform(HapticFeedbackType.LongPress);
        //for (int i = 0; i < 169; i++)
        for (int i = 0; i < 94; i++)
        ...
        await SV.ScrollToAsync(0.0, 0.0, Preferences.Get("IDScrolling", true));
